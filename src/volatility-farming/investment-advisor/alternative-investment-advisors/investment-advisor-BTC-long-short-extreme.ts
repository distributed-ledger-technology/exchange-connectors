...
export class InvestmentAdvisorBTCLongShortExtreme implements IInvestmentAdvisor {
    ...
    protected deriveSpecialMoves(investmentOption: InvestmentOption, investmentDecisionBase: any): void {
        ...
        console.log(`overallPNL: ${overallPNL} vs. oPNLClosingLimit: ${this.oPNLClosingLimit} vs. liquidityLevel: ${this.liquidityLevel}`)
    ...
    protected getAddingPointLong(): number {
        ...
        if (this.isPreviousAdviceOlderThanXMinutes(2)) {
            aPL = aPL / (this.liquidityLevel / 20)
        }

        if (this.liquidityLevel < 12) {
            aPL = aPL * (14 - this.liquidityLevel)
        }
    ...
    protected getAddingPointShort(): number {
        ...
        if (this.isPreviousAdviceOlderThanXMinutes(2)) {
            aPS = aPS / (this.liquidityLevel / 20)
        }

        if (this.liquidityLevel < 12) {
            aPS = aPS * (14 - this.liquidityLevel)
        }
